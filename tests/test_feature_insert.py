...
class TestFeatureInsert(unittest.TestCase):
    ...
    async def test_extract_xml_features(
        ...
        mock_root = ET.Element("metadata")
        package = ET.SubElement(mock_root, "package", attrib={"type": "rpm"})
        name = ET.SubElement(package, "name")
        name.text = "ansible-lint"
        ET.SubElement(
            package,
            "version",
            attrib={"epoch": "0", "ver": "4.2.0", "rel": "1.oe2403"},
        )
        url = ET.SubElement(package, "url")
        url.text = "https://github.com/ansible/ansible-lint"
        format_elem = ET.SubElement(package, "format")
        rpm_requires = ET.SubElement(
            format_elem, "{http://linux.duke.edu/metadata/rpm}requires"
        )
        for req in [
            {"name": "ansible"},
            {"name": "python(abi)"},
            ...
            {"name": "python3.11dist(ansible)"},
            {"name": "python3.11dist(pyyaml)"},
            {"name": "python3.11dist(ruamel.yaml)"},
            {"name": "python3.11dist(ruamel.yaml)"},
            {"name": "python3.11dist(setuptools)"},
            {"name": "python3.11dist(six)"},
        ]:
            ET.SubElement(
                rpm_requires,
